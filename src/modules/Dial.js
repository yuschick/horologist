...
class Dial {
  constructor(settings, parentWatch) {
    ...
    if (settings.hands.hour) {
      if (typeof settings.hands.hour === "string") {
        ...
        this.hands.hour.tenths = document.getElementById(
    ...
    if (settings.hands.minute) {
      if (typeof settings.hands.minute === "string") {
        ...
        this.hands.hour = settings.hands.minute.id;
    ...
    if (settings.hands.second) {
      ...
      } else if (typeof settings.hands.second === "object") {
    ...
    if (settings.retrograde) {
      if (settings.retrograde.second) {
        this.retrograde.second = {
          ...
          increment:
            settings.retrograde.second.max /
            (settings.retrograde.second.duration || 60),
        };
  ...
  errorChecking(settings) {
    ...
    if (
      settings.retrograde &&
      settings.retrograde.second &&
      !settings.retrograde.second.id
    ) {
      throw new ReferenceError(
  ...
  rotateHands() {
    ...
    if (this.hands.hour) {
      ...
      if (this.splitHours) {
        if (this.currentTime.hours <= 10) {
          rotateVal.ones =
            this.currentTime.hours * this.rotateValues.hoursplit.ones;
          rotateVal.tenths =
            Math.floor(this.currentTime.hours / 10) *
            this.rotateValues.hoursplit.tenths;
        } else {
          if (this.format === 24) {
            rotateVal.ones =
              (this.currentTime.hours - 10) * this.rotateValues.hoursplit.ones;
            rotateVal.tenths =
              Math.floor(this.currentTime.hours / 10) *
              this.rotateValues.hoursplit.tenths;
          } else {
            if (this.currentTime.hours > 12) {
              rotateVal.ones =
                ((this.currentTime.hours - 12) % 10) *
                this.rotateValues.hoursplit.ones;
              rotateVal.tenths =
                Math.floor((this.currentTime.hours - 12) / 10) *
                this.rotateValues.hoursplit.tenths;
            } else {
              rotateVal.ones =
                (this.currentTime.hours % 10) *
                this.rotateValues.hoursplit.ones;
              rotateVal.tenths =
                Math.floor(this.currentTime.hours / 10) *
                this.rotateValues.hoursplit.tenths;
        ...
        rotateVal =
          this.currentTime.hours * this.rotateValues.hoursRotateVal +
          this.currentTime.minutes * this.rotateValues.hoursRotateValOffset;
      }
    ...
    if (this.hands.minute) {
      ...
      if (this.splitMinutes) {
        if (this.currentTime.minutes % 10) {
          rotateVal.ones =
            this.currentTime.minutes * this.rotateValues.minutesplit.ones;
        } else {
          rotateVal =
            (this.currentTime.minutes - 10) *
            this.rotateValues.minutesplit.ones;
        }

        rotateVal.tenths =
          Math.floor(this.currentTime.minutes / 10) *
          this.rotateValues.minutesplit.tenths;
        ...
        rotateVal =
          this.currentTime.minutes * this.rotateValues.minutesRotateVal;
      }
    ...
    if (this.hands.second) {
      if (!this.isSet) {
        rotateVal =
          this.currentTime.seconds * this.rotateValues.minutesRotateVal;
        this.isSet = true;
      } else {
        rotateVal =
          this.parent.getCurrentRotateValue(this.hands.second) +
          this.rotateValues.minutesRotateVal;
      }
